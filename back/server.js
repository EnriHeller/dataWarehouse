...
server.get("/usuarios/:id",adminValidation, async (req, res) => {
    try {
...
server.put("/usuarios/:id", adminValidation, async(req,res)=>{
    ...
    const {nombre,apellido, correo, perfil,contrasena, esAdmin} = req.body;
    try {
        await Usuarios.update({nombre,apellido, correo, perfil,contrasena, esAdmin}, {where:{id: idUsuario}});
        const usuario = await Usuarios.findOne({where: {id: idUsuario}});

        if(usuario !== null){
            res.status(200).json("Contacto modificado con éxito")
        }else{
...
server.get("/regiones", async(req,res)=>{
    try {
        ...
        let arrayRegiones = [...regiones]
        if(req.user.esAdmin){
            arrayRegiones.push({av: true})
        }else{
            arrayRegiones.push({av: false})
        }
        res.status(200).json(arrayRegiones)
    } catch (error) {
        res.status(400).json(error.message)
    }
});
...
server.post("/regiones", regionValidation, async(req,res)=>{
    try {
        ...
        const regionPreExist = await Regiones.findOne({where:{nombre}})

        if(!regionPreExist){
            const nuevoElemento = await Regiones.create({
                nombre
            });
            res.status(200).json("Se ha creado con éxito la nueva región");
        }else{
...
server.put("/regiones/:id", async(req,res)=>{
    ...
    try {
        ...
        if(region !== null){
            ...
            res.status(200).json("Región modificada con éxito")
        }else{
...
server.post("/paises", paisValidation, async (req, res) => {
    try {
        ...
        const paisPreExist = await Paises.findOne({where:{nombre}})
        if(!paisPreExist){
            const nuevoPais = await Paises.create({
                nombre,
                regiones_id,
            });
            res.status(200).json("Se ha creado con éxito el nuevo país");   
        }else{
...
server.put("/paises/:id", async(req,res)=>{
    ...
    try {
        ...
        res.status(200).json("País modificado con éxito")
    } catch (error) {
        res.status(400).json({error: error.message})
    }
})
...
server.delete("/paises/:id", async (req,res) =>{
    ...
    if(!posiblePais){
        ...
        await Contactos.destroy({
            where: {ciudades_id: idCiudadesPais, paises_id: idPais}
        })
    ...
    res.status(200).json("Se ha eliminado el país con éxito");
});
...
server.get("/pais/:id", async(req,res)=>{
    const id = req.params.id
    try {
        const pais = await Paises.findOne({where:{id}});
...
server.post("/ciudades", ciudadValidation, async (req, res) => {
    try {
        ...
        const ciudadPreExist = await Ciudades.findOne({where:{nombre}})
        if(!ciudadPreExist){
            const nuevaCiudad = await Ciudades.create({
                nombre,
                paises_id,
            });
            res.status(200).json("Se ha creado con éxito la nueva ciudad");   
        }else{
...
server.put("/ciudades/:id", async(req,res)=>{
    ...
    try {
        ...
        res.status(200).json("Ciudad modificada con éxito")
    } catch (error) {
        res.status(400).json({error: error.message})
    }
})
...
server.get("/ciudad/:id", async(req,res)=>{
    const id = req.params.id
    try {
        const ciudades = await Ciudades.findOne({where:{id}});
...
//BORRAR ARRAY CONTACTOS
server.delete("/contactos", async (req,res) =>{
    try {
        const {contactsId} = req.body
        if(contactsId){
            new Promise(()=>{
                contactsId.forEach(async(id)=>{
                    const posibleContacto= await Contactos.findOne({
                        where: {
                            id,
                        }
            ...
            }).then(res.status(200).json(`Se han eliminado los contactos con éxito`))
        }else{
            throw new Error("No existe contactId")
        }
...
server.delete("/companias/:id", async (req,res) =>{
    ...
    try {
        ...
        res.status(200).json("Se ha eliminado la compañia con éxito");
    } catch (error) {
        res.status(400).json(error.message)
    }
});
...
server.post("/companias",async (req,res) =>{
    ...
    res.status(200).json("Compañía creada con éxito");}
    catch(error){
     res.status(400).json({error:error.message});
    }
});
...
server.put("/companias/:id", async(req,res)=>{
    ...
    try {
        ...
        res.status(200).json("Se ha modificado la compañía con éxito")
    } catch (error) {
        res.status(400).json({error: error.message})
    }
})
...
server.get("/companias", async(req,res)=>{
    try{
        ...
        let arrayCompanias = [...companias]
        if(req.user.esAdmin){
            arrayCompanias.push({av: true})
        }else{
            arrayCompanias.push({av: false})
        }
...
server.get("/companias/:id", async(req,res)=>{
    const id = req.params.id
    try{
        const compania = await Companias.findOne({
            where:{id},
...
server.get("/contactos", async(req,res)=>{
    ...
    let arrayContactos = [...contactos]
    if(req.user.esAdmin){
        arrayContactos.push({av: true})
    }else{
        arrayContactos.push({av: false})
    }
...
server.get("/contactos/:id", async(req,res)=>{
    const id = req.params.id
    try{
